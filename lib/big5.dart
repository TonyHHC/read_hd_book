    ...
    for (var nSrc = 0; nSrc < runes.length; nSrc += size) {
      ...
      if (r >= RUNE_SELF) {
        if (encode0Low <= r && r < encode0High) {
          r = _encode0[r - encode0Low] as int;
          if (r != 0) {
            write2(r);
            continue;
          }
        } else if (encode1Low <= r && r < encode1High) {
          r = _encode1[r - encode1Low] as int;
          if (r != 0) {
            write2(r);
            continue;
          }
        } else if (encode2Low <= r && r < encode2High) {
          r = _encode2[r - encode2Low] as int;
          if (r != 0) {
            write2(r);
            continue;
          }
        } else if (encode3Low <= r && r < encode3High) {
          r = _encode3[r - encode3Low] as int;
          if (r != 0) {
            write2(r);
            continue;
          }
        } else if (encode4Low <= r && r < encode4High) {
          r = _encode4[r - encode4Low] as int;
          if (r != 0) {
            write2(r);
            continue;
          }
        } else if (encode5Low <= r && r < encode5High) {
          r = _encode5[r - encode5Low] as int;
          if (r != 0) {
            write2(r);
            continue;
          }
        } else if (encode6Low <= r && r < encode6High) {
          r = _encode6[r - encode6Low] as int;
          if (r != 0) {
            write2(r);
            continue;
          }
        } else if (encode7Low <= r && r < encode7High) {
          r = _encode7[r - encode7Low] as int;
          if (r != 0) {
            write2(r);
            continue;
          }
